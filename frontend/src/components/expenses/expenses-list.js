...
export class ExpensesList {
    ...
    async getCategories() {
        const response = await CategoryService.getAllCategories(config.typeCategories.expenses);
    ...
    async handleConfirmDelete() {
        ...
        const response = await CategoryService.deleteCategory(config.typeCategories.expenses, id);
        if (response.error) {
            alert(response.error);
            return response.redirect ? this.openNewRoute(response.redirect) : null;
        }
